...
class SystemProcessSampler:
    def __init__(
        ...
        self._capabilities: dict[str, object] = {"gpu_proc_mem": False}
    ...
    def from_env(cls) -> "SystemProcessSampler":
        ...
        gpu_enabled = _parse_bool_env("IMONITOR_PROCESS_GPU_ENABLED", False)
        try:
    ...
    def _collect(self, ts_ns: int) -> tuple[list[dict[str, object]], dict[str, object]]:
        ...
        gpu_mem_by_pid = self._collect_gpu_proc_mem_by_pid() if self._gpu_enabled else {}
        capabilities = {"gpu_proc_mem": bool(gpu_mem_by_pid)}
        ...
        for entry in _PROC.iterdir():
            ...
            gpu_mem = float(gpu_mem_by_pid.get(pid, 0))
            gpu_known = pid in gpu_mem_by_pid
